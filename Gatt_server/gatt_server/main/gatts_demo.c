...
#define TEST_DEVICE_NAME "GATTS-SERVER"
...
void app_main(void)
{
    ...
    esp_bd_addr_t serverAddr[6] = {{0xDE, 0xCA, 0xFB, 0xEE, 0xFE, 0xD0}};

    ret = esp_ble_gap_set_rand_addr(serverAddr);
    if (ret)
    {
        ESP_LOGE(GATTS_TAG, "set rand address failed, error code = %x", ret);
        return;
    }
